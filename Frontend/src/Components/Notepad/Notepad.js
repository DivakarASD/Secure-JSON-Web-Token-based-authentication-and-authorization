...
class View extends React.Component {
	...
	loadTextsHandler = async () => {
		try {
			...
			const reply = await axios.get("http://localhost:6009/user/getTexts", {
				headers: {
	...
	render() {
		...
		let list = this.state.notes.map((note) => {
			if (note.id !== null) {
				return (
					<div
						key={1 + Math.floor(Math.random() * 10000000000000)}
						className="asdf"
					>
						...
						<form className="form-group zxcv">
							...
							<div id="select-color">
								...
								<input
									className="color-radio"
									type="radio"
									onClick={this.selectOrange}
									checked={this.state.color === "orange" ? "checked" : ""}
								/>{" "}
								Orange &nbsp;
							</div>
						</form>
						<button className="btn btn-success" onClick={this.handleAddNote}>
							Add Note
						</button>
		...
		if (this.state.inputText !== "") {
			...
			try {
				...
				const reply = await axios.post(
					"http://localhost:6009/user/addTexts",
					data,
					{
						headers: {
							"Content-Type": "application/json",
